...
// Importar os modelos
use crate::models::{Patrimony, CreatePatrimony, UpdatePatrimony};
...
pub async fn get_patrimonies(
    ...
    let mut sql = "SELECT id, plate, name, description, acquisition_date, value, department, status, invoice_number, commitment_number, denf_se_number, invoice_file, commitment_file, denf_se_file, image_url, sector, nf_issue_date, supplier, created_by, created_at, updated_at FROM patrimonies".to_string();
...
pub async fn get_patrimony(
    ...
    let result = sqlx::query(
        "SELECT id, plate, name, description, acquisition_date, value, department, status, invoice_number, commitment_number, denf_se_number, invoice_file, commitment_file, denf_se_file, image_url, sector, nf_issue_date, supplier, created_by, created_at, updated_at FROM patrimonies WHERE id = $1"
    )
    .bind(patrimony_id)
    .map(|row: PgRow| {
        let (invoice_file, commitment_file, denf_se_file, image_url) = get_document_urls(&row);
...
pub async fn create_patrimony(
    ...
    let result = sqlx::query(
        "INSERT INTO patrimonies (id, plate, name, description, acquisition_date, value, 
                ...
                sector, nf_issue_date, supplier, created_by) 
    ...
    .bind(&patrimony.invoice_number)
    .bind(&patrimony.commitment_number)
    .bind(&patrimony.denf_se_number)
    .bind(&patrimony.sector)
    .bind(&patrimony.nf_issue_date)
    .bind(&patrimony.supplier)
    .bind(user.id)
        ...
        Ok(record_id) => {
            ...
            let new_patrimony = sqlx::query(
                "SELECT id, plate, name, description, acquisition_date, value, department, status, 
                        ...
                        denf_se_file, image_url, sector, nf_issue_date, supplier, created_by, created_at, updated_at 
...
pub async fn update_patrimony(
    ...
    let existing_result = sqlx::query(
        "SELECT id, plate, name, description, acquisition_date, value, department, status, 
                invoice_number, commitment_number, denf_se_number, sector, nf_issue_date, supplier 
         FROM patrimonies WHERE id = $1"
    )
    .bind(patrimony_id)
    .fetch_one(pool.get_ref())
    ...
    let existing_invoice_number: Option<String> = existing_row.get("invoice_number");
    let existing_commitment_number: Option<String> = existing_row.get("commitment_number");
    let existing_denf_se_number: Option<String> = existing_row.get("denf_se_number");
    
    // ✅ CORREÇÃO: Para os NOVOS CAMPOS Option
    let existing_sector: Option<String> = existing_row.get("sector");
    let existing_nf_issue_date: Option<NaiveDate> = existing_row.get("nf_issue_date");
    let existing_supplier: Option<String> = existing_row.get("supplier");
    ...
    let invoice_number = patrimony.invoice_number.as_ref().or(existing_invoice_number.as_ref());
    let commitment_number = patrimony.commitment_number.as_ref().or(existing_commitment_number.as_ref());
    let denf_se_number = patrimony.denf_se_number.as_ref().or(existing_denf_se_number.as_ref());
    
    // ✅ CORREÇÃO: Para os NOVOS CAMPOS Option
    let sector = patrimony.sector.as_ref().or(existing_sector.as_ref());
    let nf_issue_date = patrimony.nf_issue_date.or(existing_nf_issue_date);
    let supplier = patrimony.supplier.as_ref().or(existing_supplier.as_ref());
    ...
    let result = sqlx::query(
        "UPDATE patrimonies SET 
            ...
            denf_se_number = $10, sector = $11, nf_issue_date = $12, supplier = $13, 
            updated_at = NOW() 
         WHERE id = $14"
    )
    .bind(plate)
...
pub async fn get_patrimonies_by_department(
    ...
    let result = sqlx::query(
        "SELECT id, plate, name, description, acquisition_date, value, department, status, invoice_number, commitment_number, denf_se_number, invoice_file, commitment_file, denf_se_file, image_url, sector, nf_issue_date, supplier,  created_by, created_at, updated_at FROM patrimonies WHERE department = $1 ORDER BY created_at DESC"
    )
