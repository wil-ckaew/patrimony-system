...
pub struct Patrimony {
    ...
    pub sector: String,          // ✅ NOVO CAMPO
    pub nf_issue_date: NaiveDate, // ✅ NOVO CAMPO
    pub supplier: String,        // ✅ NOVO CAMPO
...
pub struct UpdatePatrimony {
    ...
    pub commitment_number: Option<String>,
    pub denf_se_number: Option<String>,
    pub sector: Option<String>,          // ✅ NOVO CAMPO
    pub nf_issue_date: Option<NaiveDate>, // ✅ NOVO CAMPO
    pub supplier: Option<String>,        // ✅ NOVO CAMPO
}
...
pub async fn create_patrimony(
    ...
    .bind(&patrimony.sector)           // ✅ NOVO CAMPO
    .bind(patrimony.nf_issue_date)     // ✅ NOVO CAMPO
    .bind(&patrimony.supplier)         // ✅ NOVO CAMPO
    .bind(user.id)
...
pub async fn update_patrimony(
    ...
    let existing_result = sqlx::query(
        "SELECT id, plate, name, description, acquisition_date, value, department, status, invoice_number, commitment_number, denf_se_number, sector, nf_issue_date, supplier FROM patrimonies WHERE id = $1"
    )
    .bind(patrimony_id)
    .fetch_one(pool.get_ref())
    .await;
    ...
    let sector = patrimony.sector.as_ref().unwrap_or(&existing_row.get::<Option<String>, _>("sector").unwrap_or_default()); // ✅ NOVO CAMPO
    let nf_issue_date = patrimony.nf_issue_date.unwrap_or(existing_row.get::<NaiveDate, _>("nf_issue_date"));
    let supplier = patrimony.supplier.as_ref().unwrap_or(&existing_row.get::<Option<String>, _>("supplier").unwrap_or_default()); // ✅ NOVO CAMPO

    let result = sqlx::query(
        "UPDATE patrimonies SET plate = $1, name = $2, description = $3, acquisition_date = $4, value = $5, department = $6, status = $7, invoice_number = $8, commitment_number = $9, denf_se_number = $10, sector = $11, nf_issue_date = $12, supplier = $13, updated_at = NOW() WHERE id = $14"
    )
    .bind(plate)
    .bind(name)
    .bind(description)
    ...
    .bind(denf_se_number)
    .bind(sector)           // ✅ NOVO CAMPO
    .bind(nf_issue_date)    // ✅ NOVO CAMPO
    .bind(supplier)         // ✅ NOVO CAMPO
    .bind(patrimony_id)
