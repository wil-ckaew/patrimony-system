...
export default function AuctionDetailPage() {
  ...
  const [showVehicleDocs, setShowVehicleDocs] = useState<Set<string>>(new Set());
  ...
  const toggleVehicleDocs = (vehicleId: string) => {
    setShowVehicleDocs((prev) => {
      const next = new Set(prev);
        ...
        <div className={styles.contentCard}>
          ...
          <div className={styles.auctionInfo}>
            <div className={styles.infoGrid}>
              ...
              {/* Leiloeiro - Coluna 5 (início da linha 2) */}
              <div className={styles.infoItem}>
              ...
              {/* Observações - Coluna 7 (ocupa 2 colunas) */}
                ...
                <tbody>
                  {auction.vehicles && auction.vehicles.length > 0 ? (
                    auction.vehicles.map(vehicle => {
                      ...
                      const showDocs = showVehicleDocs.has(vehicle.id);
                      ...
                      return (
                        <React.Fragment key={vehicle.id}>
                          <tr className={styles.tableRow}>
                            ...
                            <td>
                              <div className={styles.vehicleActions}>
                                ...
                                <button
                                  className={`${styles.actionButton} ${styles.viewDocsButton}`}
                                  onClick={() => toggleVehicleDocs(vehicle.id)}
                                  title={showDocs ? "Ocultar documentos" : "Ver documentos"}
                                >
                          ...
                          {/* Documentos do veículo com botão Visualizar PDF */}
                          {showDocs && (
                            <tr className={styles.expandedRow}>
                              <td colSpan={8}>
                                <div className={styles.expandedDocs}>
                                  <h4 className={styles.docsTitle}>📄 Documentos do Veículo</h4>
                                  {auction.pdfs && auction.pdfs.length > 0 ? (
                                    <div className={styles.docsList}>
                                      {auction.pdfs.map((pdf) => (
                                        <div key={pdf.id} className={styles.docItem}>
                                          <span className={styles.docIcon}>📄</span>
                                          <span className={styles.docName}>{pdf.file_name}</span>
                                          <span className={styles.docDate}>
                                            {new Date(pdf.created_at).toLocaleDateString('pt-BR')}
                                    ...
                                    <div className={styles.emptyDocs}>
                                      <p>Nenhum documento associado a este veículo</p>
                                      <button
