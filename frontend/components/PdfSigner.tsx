...
export default function PdfSigner({ onClose, onSigned }: PdfSignerProps) {
  ...
  const [signatureRealPosition, setSignatureRealPosition] = useState({ x: 0, y: 0 });
  ...
  const handlePdfUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Posição inicial: canto inferior direito (a 100px das bordas)
  ...
  const renderPdfPage = async (pageNum: number) => {
    ...
    try {
      ...
      setContainerSize({ width: canvas.width, height: canvas.height });
      ...
      // Posicionar assinatura no canto inferior direito
      const sigX = canvas.width - signatureSize - 50;
      const sigY = canvas.height - signatureSize / 2 - 50;
      setSignaturePosition({ x: sigX, y: sigY });
      setSignatureRealPosition({ x: sigX, y: sigY });
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    if (isDragging && pdfContainerRef.current) {
      ...
      if (canvas) {
        ...
        newX = Math.max(10, Math.min(newX, canvas.width - signatureSize - 10));
        newY = Math.max(10, Math.min(newY, canvas.height - signatureSize / 2 - 10));
        
        setSignaturePosition({ x: newX, y: newY });
        setSignatureRealPosition({ x: newX, y: newY });
      }
    }
  };
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      // Calcular posição real em pixels do canvas
      const canvasW = containerSize.width || 800;
      const canvasH = containerSize.height || 600;
      
      // Posição em pixels no canvas
      const pixelX = signatureRealPosition.x || signaturePosition.x;
      const pixelY = signatureRealPosition.y || signaturePosition.y;
      ...
      const pdfY = pixelY * scaleY;

      console.log('📐 Canvas:', canvasW, 'x', canvasH);
      console.log('📐 PDF:', pdfWidth, 'x', pdfHeight);
      console.log('📍 Posição pixels:', pixelX, 'x', pixelY);
      console.log('📍 Posição PDF:', pdfX, 'x', pdfY);

      const sizeScale = signatureSize / 180;
            ...
            {!pdfFile ? (
              ...
              <>
                ...
                <div style={styles.tips}>
                  ...
                  <span>📄 Use as setas para navegar entre páginas</span>
                </div>
              </>
            )}
          </div>
