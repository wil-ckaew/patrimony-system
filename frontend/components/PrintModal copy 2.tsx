...
const PrintModal: React.FC<PrintModalProps> = ({ filters, onClose }) => {
  ...
  const fetchPatrimonies = async () => {
    ...
    try {
      ...
      const url = `http://localhost:8080/api/patrimony${
      ...
      if (response.ok) {
        ...
        if (reportType === 'byVehicle') {
          ...
          filteredData = filteredData.filter(item => {
            ...
            const hasPlate =
              item.plate &&
              item.plate.toString().trim() !== '';

            const hasVehicleType =
              item.type &&
              item.type.toString().trim() !== '';

            return hasPlate && hasVehicleType;
          });

          // Filtrar tipo selecionado
