...
export default function BulkPatrimonyForm({ onClose, onRefresh }: BulkPatrimonyFormProps) {
  ...
  // Lista de documentos fiscais múltiplos
  ...
  // Funções para múltiplos documentos fiscais
        ...
        <form onSubmit={(e) => { e.preventDefault(); executeDirectly(); }} style={styles.form}>
            ...
            {showFiscalSection && (
              <>
                ...
                <p style={{ fontSize: '12px', color: '#f59e0b', marginTop: '12px', textAlign: 'center' }}>
                  ⚠️ Todos os documentos serão anexados a TODOS os {totalItems} itens da faixa
                </p>
              </>
            )}
          </div>
          ...
          <div style={styles.sectionCard}>
            ...
            <div style={styles.formGroup}>
              ...
              />
              {image && (
                <div style={styles.filePreview}>✅ {image.name}</div>
              )}
            </div>
          </div>
          ...
          <div style={styles.actionButtons}>
            ...
            >
              {loading ? '📤 Cadastrando e enviando...' : `🚀 Cadastrar ${totalItems} Bens`}
            </button>
          </div>
          ...
          {loading && (
            <div style={{ textAlign: 'center', marginTop: '10px' }}>
              <small style={{ color: '#666' }}>
                Aguarde, anexando {image ? '1 foto + ' : ''}{fiscalDocuments.length} documento(s) fiscal(is) a todos os {totalItems} itens...
              </small>
            </div>
          )}
        </form>
